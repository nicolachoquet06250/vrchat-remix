...
// Charger les infos du projet (nom) pour SEO/Title
const { data: project, error: projError } = await useFetch(() => `/api/projects/${id.value}`)

useSeoMeta({
  title: () => `VRC Remix - ${t('analytics.title')} — ${project.value?.name ?? '#' + id.value}`,
  ogTitle: () => `${t('analytics.title')} — ${project.value?.name ?? '#' + id.value}` as string,
  description: () => t('analytics.description') as string,
  ogDescription: () => t('analytics.description') as string,
})

// Enregistrer Chart.js côté client uniquement
...
async function loadYears() {
  try {
    const res = await $fetch<{ years: number[] }>(`/api/analytics/downloads/years`, { params: { projectId: id.value } })
...
async function loadData() {
  ...
  try {
    const res = await $fetch<{
      ...
      anonymous: number[],
      all: number[]
    }>(`/api/analytics/downloads/monthly`, { params: { year: selectedYear.value, projectId: id.value } })
...
<template>
  <div class="analytics">
    <h1>{{ t('analytics.title') }} — {{ project?.name || ('#'+id) }}</h1>
    <div class="layout">
      <aside class="years">
