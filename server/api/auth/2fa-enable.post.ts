...
import { sendTwoFactorEnabledEmail } from '~~/server/utils/mail'
...
export default defineEventHandler(async (event) => {
  ...
  if (!user.twoFactorEnabled) {
    await db.update(users).set({ twoFactorEnabled: 1 }).where(eq(users.id, user.id))
    // fire-and-forget email
    sendTwoFactorEnabledEmail(user.email, { username: user.username }).catch(() => {})
  }
